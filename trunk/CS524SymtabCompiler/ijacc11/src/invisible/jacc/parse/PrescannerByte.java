...
/*->
  ...
  PrescannerByte represents an input source for the scanner.

  A PrescannerByte is required to deliver bytes into a buffer array provided
  by the scanner.
...
public interface PrescannerByte extends Prescanner
{
	...
	// Read bytes from the source.  The bytes are read into dstArray, beginning
	// at element dstOffset (i.e., the first byte read goes into array element
	// dstArray[dstOffset]).  A maximum of dstLength bytes may be read.  The
	// value of dstLength must be a positive integer.
	//
	// If the return value is positive, it is the number of bytes actually
