...
public class NST 
{
	...
	//Default
	public NST()
	{  new NST(false); } //In case someone wants to call it with no params
	
	//Primary constructor
	public NST(boolean verbose)
	{
	...
	public NSTIndScalarEntry addConstIntToCurrentBlock(String name)
	{
		...
		if (!currentBlock.entries.containsKey(name))
		{
			e = currentBlock.put(name, NST.INT_TYPE, true);
			stackTopOffset++;
			if (verbose)
			{
				...
				System.out.println("	   Adding identifier to block "+verbosityBlockCount+":");
				System.out.println("	   Name: "+name);
				System.out.println("	   Type: "+getTypeName(NST.INT_TYPE));
				System.out.println("	   StackTopOffset (address+1): "+stackTopOffset+"\n");
			}
			return e;
		}
		...
		(String name, int numParams, int startQuad, int endQuad)
	{
		...
		if (!currentBlock.entries.containsKey(name))
		{
			...
			if (verbose)
			{
				...
				System.out.println("	   Adding identifier to block "+verbosityBlockCount+":");
				System.out.println("	   Name: "+name);
				System.out.println("	   Type: "+getTypeName(NST.PROC_TYPE)+"\n");
			}
			return e;
		}
		else
...
public final class NSTBlockEntry 
{
	...
	{
		NSTIndProcEntry e =
			new NSTIndProcEntry(name,NST.PROC_TYPE,true,numParams,startQuad,endQuad);
		entries.put(name, e);
		return e;
	}
...
public static void main(String[] args)
{
	...
	NST symtab = new NST(true);
	
	symtab.startNewBlock();
	symtab.addConstIntToCurrentBlock("const1");
	symtab.addScalarToCurrentBlock("var1", NST.INT_TYPE);
	symtab.addScalarToCurrentBlock("var2", NST.INT_TYPE);
	...
	symtab.addScalarToCurrentBlock("newboolvar2", NST.BOOL_TYPE);
	symtab.addArrayToCurrentBlock("newarrayvarofint1",NST.INT_ARRAY_TYPE,5);
	symtab.addArrayToCurrentBlock("newarrayvarofbool1",NST.BOOL_ARRAY_TYPE,10);
	symtab.addScalarToCurrentBlock("var2", NST.INT_TYPE);
